...
if (jQuery("#editor").length) {
  ...
  if (jQuery("#layout1-chart-4").length) {
    ...
    $(document).ready(function() {
        if ($("#chart-product").length) {
            let options = {
                series: [{
                    name: 'Fashion',
                    data: [] // Data akan diisi dengan AJAX
                }, {
                    name: 'Gadget',
                    data: [] // Data akan diisi dengan AJAX
                }],
            ...
            const chart = new ApexCharts(document.querySelector("#chart-product"), options);
            chart.render();
            ...
            function fetchData(year) {
                $.ajax({
                    ...
                    success: function(response) {
                        const data = response.monthlyProductData;
                        chart.updateSeries([{
                            name: 'Fashion',
                            data: data.fashion
                        }, {
            ...
            $('.year-option-p').on('click', function() {
                const selectedYear = $(this).data('year'); // Mengambil data tahun dari data attribute
                fetchData(selectedYear);
                // Mengganti teks tombol dropdown dengan tahun yang dipilih
                $('#dropdownMenuButton005').html(selectedYear + '<i class="ri-arrow-down-s-line ml-1"></i>');
            });

            fetchData(2022); // Muat data awal untuk tahun 2022
        }
    });

    $(document).ready(function() {
        if ($("#chart-totalrevenue").length) {
            let options = {
                series: [{
                    name: 'Revenue',
                    data: [] // Data akan diisi dengan AJAX
            ...
            const chart = new ApexCharts(document.querySelector("#chart-totalrevenue"), options);
            chart.render();
            ...
            function fetchData(year) {
                $.ajax({
                    ...
                    success: function(response) {
                        const data = response;
                        const monthlyData = data.monthlyRevenue.map(item => item.total_revenue);
                        ...
                        const totalRevenue = data.yearlyRevenue.find(item => item.year == year).total_revenue;

                        $('#total-revenue-display').text('Rp ' + totalRevenue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
                    }
                });
            }

            $('.year-option-tr').on('click', function() {
                const selectedYear = $(this).data('year');
                fetchData(selectedYear);
                $('#dropdownMenuButtonRevenue').html(selectedYear + '<i class="ri-arrow-down-s-line ml-1"></i>');
            });
    ...
    $(document).ready(function() {
        if ($("#chart-totalincome").length) {
            ...
            const chart = new ApexCharts(document.querySelector("#chart-totalincome"), options);
            chart.render();
            ...
            function fetchData(year) {
                $.ajax({
                    ...
                    success: function(response) {
                        const data = response;
                        const monthlyData = data.monthlyIncome.map(item => item.total_income); // Corrected access to monthlyRevenue
                        chart.updateSeries([{
                            name: 'Income',
                            data: monthlyData
                        }]);
                        // Calculate the total revenue for the year
                        const totalIncome = data.yearlyIncome.find(item => item.year == year).total_income;

                        // Update the total revenue display
                        $('#total-income-display').text('Rp ' + totalIncome.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."));
                    }
                });
            }

            $('.year-option-ti').on('click', function() {
                const selectedYear = $(this).data('year');
                fetchData(selectedYear);
                $('#dropdownMenuButtonIncome').html(selectedYear + '<i class="ri-arrow-down-s-line ml-1"></i>');
            });
    ...
    $(document).ready(function() {
        if ($("#chart-product-income").length) {
            let options = {
                series: [{
                    name: 'Income',
                    data: [] // Data akan diisi dengan AJAX
            ...
            const chart = new ApexCharts(document.querySelector("#chart-product-income"), options);
            chart.render();

            function fetchData(year) {
                $.ajax({
                    url: '/get-product-data', // Ganti dengan URL endpoint yang sesuai
                    type: 'GET',
                    data: { year: year },
                    success: function(response) {
                        const data = response.monthlyProductData;
